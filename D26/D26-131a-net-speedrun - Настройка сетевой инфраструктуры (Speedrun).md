---
title: "`D26-131a-net-speedrun`: Настройка сетевой инфраструктуры (Speedrun)"
aliases:
  - "`D26-131a-net-speedrun`: Настройка сетевой инфраструктуры (Speedrun)"
  - D26-131a-net-speedrun
para: ресурс
status: задействовано
dirpipe-id: D26-131a-net-speedrun
disciplines:
...
competencies:
  ...
  - ПК 1.1
  - ПК 1.2
cdate: 2026-04-12 02:44:43
mdate: 2026-04-12 02:46:13
---
...
> 📋 **Параметры работы**
>
> *   ⏱️ **Норматив времени:** 60 минут
> *   💎 **Вес работы:** 25 из 50 баллов
> *   ⭐ **Сложность:** ⭐ Базовый
> *   📌 **Статус трека:** 🔴 Экзаменационный норматив
> *   🧭 **Педагогическая модель:** ⏱️ Скоростное прохождение (Speedrun)
> *   📝 **Описание:** Экстремально сжатый алгоритм развертывания сетевой инфраструктуры **Модуля 1** для стендов типа 🟢 **Greenfield**. Полная минимизация переключений окон, хардкод переменных и монолитная группировка команд по целевым узлам (VM-Centric).

## Вводная

Вы находитесь в режиме тактического исполнения. Дидактика, рефлексия и вариативность отключены. Данный протокол спроектирован для скоростного и безошибочного прохождения **Модуля 1**.

**Особенности скриптов Модуля 1:**
*   **Целевая среда:** Строго 🟢 **Greenfield** (имена физических интерфейсов захардкожены как `enp7s1`, `enp7s2` и т.д.).
*   **Слепое доверие:** Все пароли (`P@ssw0rd`, `toor`), интерфейсы и IP-адреса уже жестко вшиты в код согласно эталонному Техническому заданию. Ничего менять руками не нужно.
*   **Монолитность:** Конфигурация ядра (Sysctl), сетевых интерфейсов (Etcnet), фаервола (Nftables) и демонов (FRR, BIND9) выполняется единым неразрывным блоком.

> ⚠️ **Критически важно:** Правила выживания
>
> 1. **Строгий порядок обхода:** Выполняйте настройку узлов **строго** в той последовательности, в которой идут разделы этой тетради. Зависимости сетевых шлюзов не прощают хаоса.
...
> 3. **Одно окно — один скрипт:** Вы открываете консоль узла только один раз, вставляете блок целиком и больше не возвращаетесь к этой машине до конца экзамена.
> 4. **Запрет на повтор:** Скрипты не идемпотентны. Если вы запустите один и тот же блок дважды, конфигурации задублируются, и стенд гарантированно превратится в «кирпич» (Network Loop / Kernel Panic).

---

> 🎯 **Цель работы**
>
> 1. **Выполнить** сквозную настройку маршрутизатора провайдера (`ISP`) и офисных шлюзов (`HQ-RTR`, `BR-RTR`).
> 2. **Инициализировать** внутреннюю инфраструктуру: авторитетный DNS-сервер (`HQ-SRV`), клиентскую станцию (`HQ-CLI`) и сервер филиала (`BR-SRV`).
> 3. **Завершить** **Модуль 1** с полностью функциональной маршрутизацией, VPN-туннелями и ролевой моделью (SSH Hardening).
